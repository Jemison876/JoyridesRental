...
namespace JoyridesRental.Logic
{
    public class ShoppingCartActions : IDisposable
    {
        ...
        {
            using (var db =new JoyridesRental.Models.ProductContext())
            {
                try
                {
                    ...
                    foreach(var cartItem in myCart)
                    {
                        for(int i = 0; i < CartItemCount; i++)
                        {
                            if(cartItem.Product.ProductID == CartItemUpdates[i].ProductId)
                            {
                                ...
                                {
                                    UpdateItem(cartId, cartItem.ProductId, CartItemUpdates[i].PurchaseQuantity, CartItemUpdates[i].NewRentDate, CartItemUpdates[i].NewReturnDate);                                    
                                }
                            }
                        }
                    }
        ...
        public void UpdateItem(string updateCartID, int updateProductID, int quantity, string Rdate, string Retdate)
        {
            using (var db= new JoyridesRental.Models.ProductContext())
            {
                try
                {
                    ...
                    if(myItem != null)
                    {
                        ...
                        myItem.RentalDate = Rdate;
                        myItem.ReturnDate = Retdate;
                        db.SaveChanges();
                    }
                }
                catch(Exception exp)
                {
                    throw new Exception("ERROR: Unable to Update Cart Items -" + exp.Message.ToString(), exp);
                }
            }
        }
        ...
        public struct ShoppingCartUpdates
        {
            ...
            public string NewRentDate;
            public string NewReturnDate;
        }
